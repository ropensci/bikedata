...
```
where the `bikedb` parameter provides the name for the database.  The primary
objects returned by the `bikedata` packages are 'trip matrices' which contain
aggregate numbers of trips between each pair of stations. These can then be
obtained with:
```{r, eval=FALSE}
