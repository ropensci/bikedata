...
downloaded:
```{r dl_bikedata, eval=FALSE}
dl_bikedata ()
```
And stored in a `postgres` database with the following lines. The timing of
these is directly comparable to the `Rcpp` version above.
```{r store_bikedata, eval=TRUE}
ptm <- proc.time ()
