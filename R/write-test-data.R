...
bike_write_test_data <- function (data_dir = tempdir ()) {
    ...
    utils::write.csv (env$bike_test_data$ch_st,
        file = fst,
    ...
    dir.create (fdir)
    fst <- file.path (
        data_dir, "Nice_Ride_data_2012_season",
    ...
    utils::write.csv (env$bike_test_data$mn_st,
        file = fst,
        quote = FALSE, row.names = FALSE, na = ""
    )
    utils::write.csv (env$bike_test_data$mn_tr,
        file = ftr,
        quote = FALSE, row.names = FALSE, na = ""
    )
    ...
    lo$EndStation.Name <- paste0 ("\"", lo$EndStation.Name, "\"") # nolint
    lo$StartStation.Name <- paste0 ("\"", lo$StartStation.Name, "\"") # nolint
    ...
    for (i in cols) {
        env$bike_test_data$dc [[i]] <-
            as.character (env$bike_test_data$dc [[i]])
    }
    ...
    csvs <- c (
        "2017Q1-capitalbikeshare-tripdata-temp.csv",
    ...
    for (i in seq (csvs)) {
        ...
        utils::write.csv (dati,
            file = csvs [i], quote = quotes [i], na = "",
            row.names = FALSE
        )
