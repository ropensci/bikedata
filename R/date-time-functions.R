...
convert_hms <- function (x) {
    ...
    if (is.numeric (x)) {
        ...
        if (nchar (x) <= 2) { # presume it's HH
            ...
            res <- paste0 (
                substring (x, 1, 2), ":", substring (x, 3, 4),
                ":00"
            )
        } else if (nchar (x) == 6) {
...
bike_convert_dates <- function (dates) {
    ...
    if (is.numeric (dates)) {
        ...
        if (length (dates) == 1) {
            ...
            dates <- c (
                paste0 (dates [1], convert_month (dates [2])),
