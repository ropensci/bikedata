...
downloaded data and the `SQLite3` database are stored in the temporary directory
of the current `R` session. The downloaded data are deleted after having been
loaded into the `SQLite3` database, and the database itself is deleted on
termination of the `R` session.  (All of these options may be overridden as
described below.)
...
dim (dm); sum (tm)
```
between which a total of XXX individual trips up to March 2017.  For consistency
and to avoid potential confusion of function names, most functions in the
`bikedata` package begin with the prefix `bike_` (except for `store_bikedata()`
and `dl_bikedata()`).


The remainder of this vignette details these three distinct functional aspects
of downloading, storage, and extraction of data.
...
format of this argument is quite flexible so that,
```{r, eval = FALSE}
...
older stations. The `bike_tripmat()` function has an option,
`duration_standardised`, to allow trip matrices to be standardised for durations
of station operation, so that numbers of trips between any pair of stations
reflect what they would be if all stations had been in operation for the same
duration.
...
This standardisation has two immediate consequences:
...
The `duration_standardised` option nevertheless enables travel patterns between
different (groups of) stations to be statistically compared in a way that is
free of the potentially confounding influence of differing durations of
operation.
...
summary data (and which will generally do so notably faster than the above
function for large databases) include:
```{r, eval=FALSE}
