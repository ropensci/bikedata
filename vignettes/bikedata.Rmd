...
131,336 individual trips up to April 2017.  Details of the docking stations
...
```
Trip matrices can also be extracted in long form using
```{r, eval = FALSE}
tm <- bike_tripmat (bikedb = 'bikedb', city = 'la', long = TRUE)
head (tm); class (tm); sum (tm$numtrips)
```
```{r}
...
The `store_bikedata()` function will automatically download files if a
`data_dir` is not specified, in which case files will be downloaded to
`tempdir()`, and will consequently be deleted upon termination of the current
`R` session.  Enduring copies of trip files may be created with the
`dl_bikedata()` function described above. These pre-downloaded files may also be
passed directly to `store_bikedata()` by simply specifying the data directory in
the `data_dir` argument. The following call will load all previously downloaded
files in the specified directly in the nominated database.
```{r, eval = FALSE}
