...
ready importing of data from all systems provide data, and will be expanded on
...
Note that access to data from the Santander Cycles system of London,
U.K. requires an initial registration in order to obtain an `OAuth` key to
access the data.
...
Angeles, CA and store it in an `SQLite3` database. The function returns the
number of trips added to the database. By default, both the downloaded data and
the `SQLite3` database are stored in the temporary directory of the current `R`
session. The downloaded data are deleted after having been loaded into the
`SQLite3` database, and the database itself is deleted on termination of the `R`
session.  (All of these options may be overridden as described below.)
...
`bikedata` package is a trip matrices containing aggregate numbers of trips
