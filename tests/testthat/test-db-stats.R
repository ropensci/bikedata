...
test_that ("can not read all data", {
    expect_error (
        trips <- bike_tripmat (bikedb),
        "bikedb contains multiple cities; please specify one"
    )
    expect_error (
        trips <- bike_tripmat (bikedb, city = "aa"),
        "city not recognised"
    )
})
...
test_that ("dplyr read db", {
    ...
    nms <- c (
        "id", "city", "trip_duration", "start_time",
...
test_that ("db stats", {
    if (test_all) { # summary_stats checks latest_files too
        ...
        if (!is.null (db_stats)) {
            ...
            expect_true (all (db_stats$city == c (
                "total", "bo",
                "ch", "dc", "la",
                "lo", "ny"
            )))
