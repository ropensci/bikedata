...
test_that ('tripmat-full', {
               expect_silent (tm <- bike_tripmat ("testdb", quiet = TRUE))
               expect_equal (dim (tm), c (9, 9))
               expect_equal (sum (tm), 162)
})

test_that ('tripmat-startday', {
               expect_silent (tm <- bike_tripmat ("testdb",
                                                  start_date = 20150102,
                                                  quiet = TRUE))
               expect_equal (dim (tm), c (9, 9))
...
test_that ('tripmat-starttime', {
               expect_silent (tm <- bike_tripmat ("testdb", start_time = 1,
                                                  quiet = TRUE))
               expect_equal (sum (tm), 140)
               expect_silent (tm2 <- bike_tripmat ("testdb", start_time = "1",
                                                   quiet = TRUE))
               expect_silent (tm3 <- bike_tripmat ("testdb",
                                                   start_time = "01:00",
                                                   quiet = TRUE))
...
test_that ('tripmat-endtime', {
               expect_silent (tm <- bike_tripmat ("testdb", end_time = 4,
                                             quiet = TRUE))
               expect_equal (sum (tm), 88)
               expect_silent (tm2 <- bike_tripmat ("testdb", end_time = "4",
                                             quiet = TRUE))
               expect_silent (tm3 <- bike_tripmat ("testdb", end_time = "4:00",
                                             quiet = TRUE))
               expect_identical (tm, tm2)
               expect_identical (tm, tm3)
})
...
test_that ('tripmat-endtime-enddate', {
               expect_silent (tm <- bike_tripmat ("testdb", end_date = 20150103,
...
test_that ('weekday', {
               expect_silent (tm <- bike_tripmat ("testdb", weekday = 5:6,
                                                  quiet = TRUE))
               expect_equal (sum (tm), 81)
               expect_silent (tm <- bike_tripmat ("testdb",
                                                  weekday = c('f', 'sa'),
                                                  quiet = TRUE))
               expect_equal (sum (tm), 81)
               expect_error (tm <- bike_tripmat ("testdb",
                                                 weekday = c('f', 's'),
                                                 quiet = T),
